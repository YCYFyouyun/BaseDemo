...
public class AdvancedWebView extends WebView {
    ...
    protected void openFileInput(final ValueCallback<Uri> fileUploadCallbackFirst, final ValueCallback<Uri[]> fileUploadCallbackSecond, final boolean allowMultiple) {
        ...
        if (mFragmentReference != null && mFragmentReference.get() != null && Build.VERSION.SDK_INT >= 11) {
            mFragmentReference.get().startActivityForResult(Intent.createChooser(i, getFileUploadPromptLabel()), mRequestCodeFilePicker);
        } else if (mActivityReference != null && mActivityReference.get() != null) {
            mActivityReference.get().startActivityForResult(Intent.createChooser(i, getFileUploadPromptLabel()), mRequestCodeFilePicker);
        }
    }
     ...
     * Returns whether file uploads can be used on the current device (generally all platform versions except for 4.4)
     * <p>
     * On Android 4.4.3/4.4.4, file uploads may be possible but will come with a wrong FROM_MIME type
     *
     * @param needsCorrectMimeType whether a correct FROM_MIME type is required for file uploads or `application/octet-stream` is acceptable
     * @return whether file uploads can be used
     */
